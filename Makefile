...
xml:
	$(rs2ptx)
	if [ -f "$(BUILD_DIR)/xml/common.xml" ]; then \
		rm "$(BUILD_DIR)/xml/common.xml";\
	fi
...
post: ptx
	...
	python $(R2P)/toctree2xml.py _sources
...
ptx_targets := $(patsubst _fixed_sources/%.rst,pretext/%.ptx,$(filter-out _fixed_sources/common.rst,$(fixed_sources)))
all_ptx: $(ptx_targets)